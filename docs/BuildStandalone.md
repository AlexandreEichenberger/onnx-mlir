...
- `-DLLVM_ENABLE_ZLIB=OFF` disables zlib compression support to avoid libz dependency
- `-DLLVM_ENABLE_ZSTD=OFF` disables zstd compression support to avoid a Homebrew dependency on macOS

**Step 2: Build onnx-mlir as standalone**
```bash
# Configure with standalone mode
MLIR_DIR=$(pwd)/llvm-project/build/lib/cmake/mlir
rm -rf onnx-mlir/build
mkdir onnx-mlir/build && cd onnx-mlir/build
if [[ -z "$pythonLocation" ]]; then
  cmake -G "Unix Makefiles" -DCMAKE_CXX_COMPILER=/usr/bin/c++ \
    -DMLIR_DIR=$MLIR_DIR \
    -DCMAKE_BUILD_TYPE=Release \
    -DLLVM_ENABLE_ASSERTIONS=ON \
...
- **macOS:** Use `-DCMAKE_IGNORE_PATH="/opt/homebrew;/usr/local"` or `-DCMAKE_IGNORE_PATH="/usr/local;/opt"` depending on your Homebrew installation
- First build will take longer (~10-20 minutes extra) as it builds protobuf and abseil from source
- For NNPA accelerator support, add `-DONNX_MLIR_ACCELERATORS=NNPA` to the cmake command
